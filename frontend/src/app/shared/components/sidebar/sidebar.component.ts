...
@Component({
  ...
  template: `
    <aside class="sidebar" [class.open]="layout.sidebarOpen()">
      <a class="sidebar-brand" [routerLink]="homePath()" (click)="layout.closeSidebar()">
        <app-logo [size]="32" />
      </a>
      ...
      <nav class="sidebar-nav" aria-label="Navigation principale">
        @if (isTenant()) {
          ...
          <a
            routerLink="/espace-locataire/documents"
            routerLinkActive="active"
            (click)="layout.closeSidebar()"
          >
            <svg
          ...
          <a routerLink="/alerts" routerLinkActive="active" (click)="layout.closeSidebar()">
            ...
            @if (alerts.unreadCount() > 0) {
              <span class="nav-badge">{{ alerts.unreadCount() }}</span>
            }
          </a>
        } @else {
          ...
          <a routerLink="/documents" routerLinkActive="active" (click)="layout.closeSidebar()">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
          ...
          <a routerLink="/reports" routerLinkActive="active" (click)="layout.closeSidebar()">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
              height="20"
...
export class SidebarComponent {
  ...
  /**
   * Le locataire n'a pas de parc à naviguer.
   *
   * Lui présenter « Portefeuilles » et « Rapports » ne serait pas seulement
   * inutile : chacune de ces pages s'ouvrirait sur du vide, ce qui ressemble à
   * une panne plutôt qu'à une absence de droits.
   */
  protected readonly isTenant = computed(() => this.auth.currentUser()?.role === 'locataire');

  /** Racine de la marque : chaque profil rentre chez lui. */
  protected readonly homePath = computed(() => this.auth.homePath());
}
